...
const initialValues = { department_id:"", source_person_id:"", first_name:"", middle_name:"", surname:"", suffix:"", birth_date:"", sex:"", email:"", mobile_number:"", employment_status:"", highest_education:"", preferred_occupation:"", source_data:"", requester_name:"", institution_office:"", research_title_purpose:"", category:"", control_number:"", date_received:"", status:"received", decision_notes:"", person_name:"", personnel_number:"", occurred_at:"", attendance_status:"", external_location_id:"", employment_id_number:"", workcode:"", verify_code:"", card_number:"" };
...
export function AddRecordModal({ database, onClose, onCreated }: { database: DatabaseKey; onClose: () => void; onCreated: (message: string) => void }) {
  ...
  const submit=async(event:FormEvent)=>{
    ...
    for(const line of values.source_data.split(/\r?\n/).map(item=>item.trim()).filter(Boolean)){
      const separator=line.indexOf(":");
      if(separator<1){setError("Use one additional NSRP field per line in the format Field: value.");setSaving(false);return;}
