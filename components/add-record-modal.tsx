...
import { CaretLeft, CaretRight, CheckCircle, IdentificationCard, SpinnerGap, X } from "@phosphor-icons/react";
...
const jobseekerSteps=["Identity","Address","Employment","Education","Credentials","Experience"];
...
export function AddRecordModal({ database, onClose, onCreated }: { database: DatabaseKey; onClose: () => void; onCreated: (message: string) => void }) {
  ...
  const [jobseekerStep,setJobseekerStep]=useState(0);
  ...
  const nextJobseekerStep=()=>{
    if(!formRef.current?.reportValidity())return;
    setJobseekerStep(current=>Math.min(current+1,jobseekerSteps.length-1));
    formRef.current?.scrollTo({top:0,behavior:"smooth"});
  };
  const previousJobseekerStep=()=>{
    setJobseekerStep(current=>Math.max(current-1,0));
    formRef.current?.scrollTo({top:0,behavior:"smooth"});
  };
  ...
  return <div className="modal-backdrop record-modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!saving)onClose()}}>
    <section ref={dialogRef} className="record-modal" role="dialog" aria-modal="true" aria-labelledby="add-record-title" aria-describedby="add-record-description">
      ...
      {database==="jobseekers"&&<nav className="jobseeker-stepper" aria-label="Jobseeker form progress">{jobseekerSteps.map((label,index)=><div className={index===jobseekerStep?"active":index<jobseekerStep?"complete":""} key={label}><span>{index<jobseekerStep?<CheckCircle weight="fill"/>:index+1}</span><small>{label}</small></div>)}</nav>}
      <form ref={formRef} onSubmit={submit} className="record-modal-form">
        {(database!=="jobseekers"||jobseekerStep===0)&&<div className="record-form-section"><div className="record-section-heading"><span>01</span><div><h3>Record ownership</h3><p>Choose the municipal department responsible for this record.</p></div></div><div className="record-form-grid"><label className="span-2">Department <b>*</b><select autoFocus value={values.department_id} onChange={e=>update("department_id",e.target.value)} required disabled={loadingDepartments}><option value="">{loadingDepartments?"Loading departments…":"Select department"}</option>{departments.map(item=><option key={item.id} value={item.id}>{item.name} ({item.code})</option>)}</select></label></div></div>}
        {database==="jobseekers"&&<JobseekerFields step={jobseekerStep} values={values} update={update} details={jobseekerDetails} updateDetail={updateJobseekerDetail}/>}
        ...
        <footer className="record-modal-actions"><span><CheckCircle/> {database==="jobseekers"?`Step ${jobseekerStep+1} of ${jobseekerSteps.length}`:"Required fields are marked with *"}</span><div>{database==="jobseekers"&&jobseekerStep>0&&<button type="button" className="button secondary" onClick={previousJobseekerStep} disabled={saving}><CaretLeft/> Back</button>}<button type="button" className="button secondary" onClick={onClose} disabled={saving}>Cancel</button>{database==="jobseekers"&&jobseekerStep<jobseekerSteps.length-1?<button type="button" className="button primary" onClick={nextJobseekerStep} disabled={loadingDepartments}>Continue <CaretRight/></button>:<button type="submit" className="button primary" disabled={saving||loadingDepartments}>{saving?<><SpinnerGap className="spin"/> Saving record…</>:"Add record"}</button>}</div></footer>
      </form>
    </section>
  </div>;
}
...
type JobseekerFieldsProps=FieldsProps & {step:number;details:Record<string,string>;updateDetail:(field:string,value:string)=>void};
...
function DetailSection({number,title,description,fields,details,updateDetail}:{number:string;title:string;description:string;fields:DetailField[];details:Record<string,string>;updateDetail:(field:string,value:string)=>void}){return <section className="record-form-section"><div className="record-section-heading"><span>{number}</span><div><h3>{title}</h3><p>{description}</p></div></div><div className="record-form-grid detail-grid">{fields.map(field=><label className={field.wide?"span-2":""} key={field.key}><span className="field-label">{field.label}{field.required&&<b> *</b>}</span>{field.options?<select value={details[field.key]??""} onChange={e=>updateDetail(field.key,e.target.value)} required={field.required}><option value="">Select</option>{field.options.map(option=><option key={option}>{option}</option>)}</select>:<input type={field.type??"text"} value={details[field.key]??""} onChange={e=>updateDetail(field.key,e.target.value)} required={field.required} maxLength={500}/>}</label>)}</div></section>}

function RepeatableSection({number,title,description,entryLabel,fields,count,details,updateDetail}:{number:string;title:string;description:string;entryLabel:string;fields:string[];count:number;details:Record<string,string>;updateDetail:(field:string,value:string)=>void}){return <section className="record-form-section"><div className="record-section-heading"><span>{number}</span><div><h3>{title}</h3><p>{description}</p></div></div>{Array.from({length:count},(_,index)=><fieldset className="repeatable-fields" key={index}><legend>{entryLabel} {index+1}</legend><div className="record-form-grid">{fields.map(field=>{const key=`${field} ${index+1}`;return <label key={key}><span className="field-label">{field}</span><input value={details[key]??""} onChange={e=>updateDetail(key,e.target.value)} maxLength={500}/></label>})}</div></fieldset>)}</section>}

function JobseekerFields({step,values,update,details,updateDetail}:JobseekerFieldsProps){
  if(step===0)return <section className="record-form-section"><div className="record-section-heading"><span>02</span><div><h3>Personal details</h3><p>Required identity and contact information from NSRP Form 1.</p></div></div><div className="record-form-grid"><label><span className="field-label">First name <b>*</b></span><input value={values.first_name} onChange={e=>update("first_name",e.target.value)} maxLength={120} required/></label><label><span className="field-label">Middle name</span><input value={values.middle_name} onChange={e=>update("middle_name",e.target.value)} maxLength={120}/></label><label><span className="field-label">Surname <b>*</b></span><input value={values.surname} onChange={e=>update("surname",e.target.value)} maxLength={120} required/></label><label><span className="field-label">Suffix</span><input value={values.suffix} onChange={e=>update("suffix",e.target.value)} maxLength={30} placeholder="Jr., Sr., III"/></label><label><span className="field-label">Date of birth <b>*</b></span><input type="date" value={values.birth_date} onChange={e=>update("birth_date",e.target.value)} required/></label><label><span className="field-label">Gender <b>*</b></span><select value={values.sex} onChange={e=>update("sex",e.target.value)} required><option value="">Select</option><option>Female</option><option>Male</option><option>LGBTQIA+</option></select></label><label><span className="field-label">Email address</span><input type="email" value={values.email} onChange={e=>update("email",e.target.value)} maxLength={254}/></label><label><span className="field-label">Contact number <b>*</b></span><input value={values.mobile_number} onChange={e=>update("mobile_number",e.target.value)} maxLength={40} required/></label><label className="span-2"><span className="field-label">Source person ID</span><input value={values.source_person_id} onChange={e=>update("source_person_id",e.target.value)} maxLength={80} placeholder="Optional external identifier"/></label></div></section>;
  if(step===1)return <DetailSection number="03" title="Address and profile" description="Personal profile, address, and disability information." fields={personalDetails} details={details} updateDetail={updateDetail}/>;
  if(step===2)return <><section className="record-form-section"><div className="record-section-heading"><span>04</span><div><h3>Employment preferences</h3><p>These values feed the concise Records preview.</p></div></div><div className="record-form-grid"><label><span className="field-label">Employment status <b>*</b></span><select value={values.employment_status} onChange={e=>update("employment_status",e.target.value)} required><option value="">Select status</option><option>Employed</option><option>Unemployed</option></select></label><label><span className="field-label">Highest educational attainment</span><input value={values.highest_education} onChange={e=>update("highest_education",e.target.value)} maxLength={200}/></label><label className="span-2"><span className="field-label">Preferred occupation</span><input value={values.preferred_occupation} onChange={e=>update("preferred_occupation",e.target.value)} maxLength={200}/></label></div></section><DetailSection number="05" title="Employment and household" description="Employment type, job search, 4Ps, and OFW history." fields={employmentDetails} details={details} updateDetail={updateDetail}/><DetailSection number="06" title="Work preferences" description="Preferred employment, locations, countries, and languages." fields={preferenceDetails} details={details} updateDetail={updateDetail}/></>;
  if(step===3)return <DetailSection number="07" title="Education" description="Schools, strands, programs, and graduation years." fields={educationDetails} details={details} updateDetail={updateDetail}/>;
  if(step===4)return <><RepeatableSection number="08" title="Training" description="Record up to three technical or vocational trainings." entryLabel="Training" fields={["TRAINING/VOCATIONAL COURSE","HOURS OF TRAINING","TRAINING INSTITUTION","SKILLS ACQUIRED","CERTIFICATE RECEIVED"]} count={3} details={details} updateDetail={updateDetail}/><RepeatableSection number="09" title="Civil service eligibility" description="Record civil-service eligibility entries and their examination dates." entryLabel="Eligibility" fields={["TYPE OF CIVIL SERVICE ELIGIBILITY?","DATE TAKEN"]} count={2} details={details} updateDetail={updateDetail}/><RepeatableSection number="10" title="Professional licenses" description="Record up to four professional licenses and their expiry dates." entryLabel="License" fields={["PROFESSIONAL LICENSE","EXPIRY DATE/ VALID UNTIL"]} count={4} details={details} updateDetail={updateDetail}/></>;
  return <><RepeatableSection number="11" title="Work experience" description="Record up to five employers, positions, and employment periods." entryLabel="Work experience" fields={["COMPANY NAME","COMPANY ADDRESS (City/Municipality)","POSITION/ JOB TITLE","NUMBER OF MONTHS","STATUS"]} count={5} details={details} updateDetail={updateDetail}/><DetailSection number="12" title="Skills" description="List all skills that apply to this jobseeker." fields={[textField("SELECT ALL SKILLS THAT APPLY","Skills",false,true)]} details={details} updateDetail={updateDetail}/></>;
}
